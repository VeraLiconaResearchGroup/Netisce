...
**A Note about R packages**
NETISCE was originally constructed and run using R version 3.6.3. However, it has also been tested and functions in R version 4.0.4. The in `NETISCE_hpc` we specify for R scripts to use R/3.6.3 and have included the relevant R packages within the `bin` folder in an effort to alleviate potential package installation issues when using an hpc. 
