...
\tightlist
\item
  \href{https://scipy.org/install/}{scipy} version=1.5.4
\item
  \href{https://pandas.pydata.org/getting_started.html}{pandas} version=1.1.5
\item
  \href{https://scikit-learn.org/stable/install.html}{sklearn} version=0.24.2
\item
  \href{https://www.scikit-yb.org/en/latest/quickstart.html}{yellowbrick} version=1.3.post1
\end{itemize}
...
\textbf{A Note about R packages}
NETISCE was originally constructed and run using R version 3.6.3. However, it has also been tested and functions in R version 4.0.4. The in \texttt{NETISCE\_hpc} we specify for R scripts to use R/3.6.3 and have included the relevant R packages within the \texttt{bin} folder in an effort to alleviate potential package installation issues when using an hpc.
...
\item
  \textbf{params.randseed}: random seed to generate one FVS of a network. \textbf{Note:} If you are interested in identifying all FVSes within a network, and using this information to specifically select a FVS, please see the additional folder and Nextflow script in the \texttt{FVS\_finding} folder. Detailed description of the input and output is below.
\end{itemize}
...
\hypertarget{fvs_finding-nextflow-script}{%
\section{FVS\_Finding Nextflow Script}\label{fvs_finding-nextflow-script}}
...
\textbf{params.network} = location of network file (this can be directed to your \texttt{input\_data} folder)

\textbf{params.sets} = number of FVS searches to perform (i.e., the randomseed value). This should be set reasonably high to discover all FVSes within a network, based on the network size.

\#\#output

\textbf{FVSes.txt} = file containing the unique minimal FVSes within the network. The first column contains the FVS identifier- the number following ``FVS\_'' indicates which randomseed value was used to identify that specific FVS. You can use this value as the setting for \textbf{params.randseed} in NETISCE.
...
\hypertarget{section-id}{%
\subsubsection*{experimental\_internalmarkers.png}\label{section-id}}
\addcontentsline{toc}{subsubsection}{experimental\_internalmarkers.png}
...
\addcontentsline{toc}{subsubsection}{pert\_replicate\_i\_internal\_markers.txt}
...
\hypertarget{fvs_finding-output-of-fvses.txt}{%
\subsubsection{FVS\_Finding output of FVSes.txt}\label{fvs_finding-output-of-fvses.txt}}

The \texttt{FVS\_Finding} Nextflow script was used to identify all unique FVSes in the toy network.
...
The first column contains the FVS identifier, where the number is the \texttt{randomseed} value where that FVS was identified. There were four FVSes identified in this network, each comprised of two nodes (column2 and column 3)

\hypertarget{section-id}{%
\subsubsection*{fvs.txt}\label{section-id}}
\addcontentsline{toc}{subsubsection}{fvs.txt}

We selected FVS\_0 to be analyzed in this example. Therefore, \texttt{params.randomseed} was set to 0
...
Let's take a look at the steady-state expression values of node C in the attractors generated from the successful perturbations and the experimental initial states B\_1 when mutational information is included.
...
\begin{figure}
\includegraphics[width=0.5\linewidth]{ipsc/results/elbow} \includegraphics[width=0.5\linewidth]{ipsc/results/silhouette} \caption{optimal k as identified by a) the elbow and b) silhouette metrics}\label{fig:unnamed-chunk-26}
\end{figure}
...
\caption{histograms of 4 internal-marker node values}\label{fig:unnamed-chunk-29}
\end{figure}
...
\caption{histograms of 7 internal-marker node values}\label{fig:unnamed-chunk-32}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.5\linewidth]{crc/results/elbow} \includegraphics[width=0.5\linewidth]{crc/results/silhouette} \caption{optimal k as identified by a) the elbow and b) silhouette metrics}\label{fig:unnamed-chunk-37}
\end{figure}
...
\caption{histograms of 3 internal-marker node values}\label{fig:unnamed-chunk-40}
\end{figure}
...
\caption{the precentage of specific perturbations on FVS control nodes in the perturbations that passed the filtering criteria}\label{fig:unnamed-chunk-42}
\end{figure}
...
\includegraphics[width=0.5\linewidth]{_main_files/figure-latex/crc-charts-1} \includegraphics[width=0.5\linewidth]{_main_files/figure-latex/crc-charts-2} \textbackslash caption\{radar charts of the steady-state values of 3 internal-marker nodes for HT29\_BRAFi+SRCi\}\label{fig:crc-charts}
