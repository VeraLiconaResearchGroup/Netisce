...
The nextflow pipeline generates 1 result file \texttt{exp\_internalmarkers.txt}, which contains the steady state values of the internal-marker nodes for the unperturbed attractor, and the attractors generated from the perturbations on FVS control nodes.
\# \texttt{\{r,echo=FALSE,\ message=FALSE,\ warning=FALSE\}\ \#\ library(dplyr)\ \#\ library(kableExtra)\ \#\ \ \#\ results\textless{}-read.delim(\textquotesingle{}ascidian\_embryo/results/exp\_internalmarkers.txt\textquotesingle{},sep="\textbackslash{}t")\ \#\ knitr::kable(results)\ \%\textgreater{}\%kable\_styling(font\_size\ =\ 8)\ \%\textgreater{}\%\ scroll\_box(width\ =\ "100\%",\ box\_css\ =\ "border:\ 0px;")\ \#}
