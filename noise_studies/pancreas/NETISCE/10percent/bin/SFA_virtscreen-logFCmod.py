...
FC_logFC=pd.read_csv(argv[8],usecols=['gene_name','log2FoldChange'],index_col='gene_name',header=0)
    ...
    for name, item in samples.iteritems():                      #for each simulated initial condition
        ...
        for name2,item2 in FC_perts.iterrows():                  # for each FCnode perturbation
            ...
            for node in pnode: #if logFC.loc[node].at['logFC']>0: # if the logfc is postive (res > veh)
                if item2.loc[node]==-1:          #if the perturbation is downregulation
                    if node in item.index:  # if there is an expression value for the node
                        if FC_logFC.loc[node,'log2FoldChange']<0: # if the logfc is negative
                            b[data.n2i[node]]=float(str(item.loc[node]))-2.5*(float(str(item.loc[node])))  #set to donwregulation
                            pi.append(data.n2i[node])
                        else: # if logFC is positive
                            b[data.n2i[node]]=float(str(item.loc[node])) #do not change value
                    else: #if there is not an expression value
                        b[data.n2i[node]]=-2.5        #fix to 0
                        pi.append(data.n2i[node])
                if item2.loc[node]==1:          #if the perturbation is overexpression
                    if node in item.index:  # if there is an expression value for the node
                        if FC_logFC.loc[node,'log2FoldChange']>0: # if the logfc is positive
                            b[data.n2i[node]]=float(str(item.loc[node]))+2.5*(float(str(item.loc[node]))) #upregulate the expression
                            pi.append(data.n2i[node])
                        else: #logFC is negative
                            b[data.n2i[node]]=float(str(item.loc[node])) #do not change value                       
                    else: # no expession information
